			   ...
			   int num)
{
	...
	reqs = dma_alloc_coherent(
		vq->vdev->dev.parent, sizeof(struct virtio_i2c_req) * num, &dma_handle, GFP_KERNEL);
...
err_free:
	dma_free_coherent(
		vq->vdev->dev.parent, sizeof(struct virtio_i2c_req) * num, reqs, dma_handle);
	return count;
}
